...
The main loop and input routing live in `src/game/Game.js`. `src/game/world/Garage.js` owns the world layout, bay and road hit zones, validated route construction, and environment rendering. `src/game/entities/Vehicle.js` owns each car's state and procedural overlays, while `src/game/core/VehicleMotion.js` follows cubic driving paths and validates their curvature and heading continuity. `Game.js` orchestrates the tap-tow-truck-then-parked-car rescue, and `src/game/entities/TowTruck.js` owns the truck's activation, hook, beacon, and rescue state. Icon-only controls live in `src/game/ui/Hud.js`, and small safe car surprises live in `src/game/actions/vehicleSurprises.js`.
...
Tow rescue follows the same contract. Tapping the tow truck activates target selection; tapping a parked car sends the truck along validated driving paths to back up to it, attach the hook, tow it safely offscreen, and return home. The truck and the hooked car must each move through their own `vehicle.drive()` and `motion.follow()` path. Keeping the hook visually connected is not permission to assign either vehicle's pose directly, skip validation, teleport through a rescue segment, or ignore the normal collision coordinator.
...
There is no score, loss condition, reading requirement, or dead end. When a compatible bay is open and no car is already waiting, the game automatically brings one new car in from the road and keeps it available for the player to direct. There is no entrance door, bell, or arrival button. Parking, reparking, road exits, booth pickups, and tow rescues remain player-caused. Automatic arrival is driven by parking availability, not by an idle timer. Timers may create harmless parked-car charm, but they must never create vehicle problems or pickup requests. The tow truck has one complete, readable interaction: tap it to activate it, then tap a parked vehicle so the truck backs up, hooks it, tows it away, and returns home. Service-station trips and an all-done celebration remain deferred; do not expose their dormant routes or pretend those features work. Station assets and hit zones stay hidden until their validated driving arcs and interactions are ready to ship together.
...
Clear `localStorage` and reload for a fresh world. Acceptance checks should cover automatic roadside arrival on a fresh world, replenishment when another compatible bay becomes available, parking, both repark directions, concurrent yielding with zero body overlap, direct road exit, booth pickup, a complete tow rescue and tow-truck return, day and night, and the one-shot parked-car idle charm. Confirm that no door, arrival button, cat, or unfinished station affordance is visible or interactive. During tow acceptance, verify that selection is obvious, only a parked car can become the target, all tow segments pass path validation, the hooked pair never overlaps another vehicle, and the next automatic arrival waits until the rescued vehicle has cleared the playfield. Do not inspect a removed vehicle's retained test object as though it were still colliding; count overlap only while both objects remain in `game.vehicles`.
