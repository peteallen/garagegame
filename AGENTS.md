...
There is no score, loss condition, reading requirement, or dead end. The entrance creates at most one arrival or waiting car at a time. Parking, reparking, road exits, and booth pickups are player-caused. Before traffic starts, the cat visibly clears through the apron corridor to its refuge and remains there until movement finishes; never remove that traffic hold or allow a moving vehicle to overlap `Pet.footprint()`. Timers may create harmless idle charm, but they must never create arrivals, vehicle problems, or pickup requests. Service-station trips, tow rescue, and an all-done celebration are deferred; do not expose their dormant routes or pretend those features work. The station assets and hit zones stay hidden until their validated driving arcs and interactions are ready to ship together.
...
Clear `localStorage` and reload for a fresh world. Acceptance checks should cover arrival, parking, both repark directions, concurrent yielding with zero body overlap, direct road exit, booth pickup, day and night, cat roof dismount, and the one-shot idle charm. Do not inspect a removed vehicle's retained test object as though it were still colliding; count overlap only while both objects remain in `game.vehicles`.
